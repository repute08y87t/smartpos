...
const turso = createClient({
    url: process.env.TURSO_DATABASE_URL || 'libsql://smartpos-repute08y87t.aws-ap-northeast-1.turso.io',
    authToken: process.env.TURSO_AUTH_TOKEN || 'eyJhbGciOiJFZERTQSIsInR5cCI6IkpXVCJ9.eyJhIjoicnciLCJpYXQiOjE3Nzg2MDU5NzAsImlkIjoiMDE5ZTFkMmMtOTkwMS03YjhmLThlMDktMGU4OTkxZmU5ZDhmIiwicmlkIjoiMWE5MjkxOWItMDEyOS00ZGMwLTk5YzYtYjVjMjRmODM3Njc3In0.cDpRVWn-3GiLMtcgnrNBaB-wgcwBYAFq_tV2pKiQdCbR2VfinaIRRDm8tekVtqYUzFcc7LYdX0ICEHPEPLBDDg'
});

// Initialize database schema - split statements for Turso
async function initDatabase() {
    const initSql = fs.readFileSync(path.join(__dirname, 'data', 'init_fixed.sql'), 'utf8');
    const statements = initSql.split(/;\s*\n/).filter(s => s.trim().length > 0);
    ...
    console.log("Database ready on Turso");
}
initDatabase();
...
app.get('/api/health', (req, res) => {
    res.json({ status: 'ok', message: 'SmartPOS running on Turso' });
});
...
app.post('/api/products', async (req, res) => {
    ...
    try {
        ...
        res.json({ id: Number(result.lastInsertRowId), message: 'Product added' });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});
...
app.put('/api/categories/edit', async (req, res) => {
    ...
    try {
        await runQuery('UPDATE categories SET name = ? WHERE name = ?', [new_name, old_name]);
        await runQuery('UPDATE products SET category = ? WHERE category = ?', [new_name, old_name]);
...
app.delete('/api/categories/delete', async (req, res) => {
    ...
    try {
        await runQuery('DELETE FROM categories WHERE name = ?', [category]);
        await runQuery('UPDATE products SET category = "General" WHERE category = ?', [category]);
...
app.post('/api/subcategories', async (req, res) => {
    ...
    try {
        ...
        if (catResult.rows.length === 0) {
            ...
            await runQuery('INSERT OR IGNORE INTO subcategories (category_id, name) VALUES (?, ?)', [catId, name]);
...
app.put('/api/subcategories/edit', async (req, res) => {
    ...
    try {
        ...
        if (catResult.rows.length === 0) {
            ...
            const catId = catResult.rows[0].id;
            await runQuery('UPDATE subcategories SET name = ? WHERE category_id = ? AND name = ?', [new_name, catId, old_name]);
            await runQuery('UPDATE products SET subcategory = ? WHERE category = ? AND subcategory = ?', [new_name, category, old_name]);
...
app.delete('/api/subcategories/delete', async (req, res) => {
    ...
    try {
        ...
        if (catResult.rows.length === 0) {
            ...
            const catId = catResult.rows[0].id;
            await runQuery('DELETE FROM subcategories WHERE category_id = ? AND name = ?', [catId, subcategory]);
            await runQuery('UPDATE products SET subcategory = "" WHERE category = ? AND subcategory = ?', [category, subcategory]);
...
app.post('/api/brands', async (req, res) => {
    ...
    try {
        ...
        await runQuery('INSERT OR IGNORE INTO brands (category_id, subcategory_id, name) VALUES (?, ?, ?)', [catId, subcatId, name]);
...
app.post('/api/sales/complete', async (req, res) => {
    ...
    if (method === 'split' && splitMpesa > 0 && splitCash >= 0) {
        try {
            await runQuery(`INSERT INTO sales (receipt_no, subtotal, total, status, payment_timestamp, method) VALUES (?, ?, ?, 'completed', ?, 'split')`, [receipt_no, total, total, finalTimestamp]);
            await runQuery(`INSERT INTO sales (receipt_no, subtotal, total, status, payment_timestamp, method, parent_receipt) VALUES (?, ?, ?, 'completed', ?, 'mpesa', ?)`, [receipt_no + '-MPESA', splitMpesa, splitMpesa, finalTimestamp, receipt_no]);
            await runQuery(`INSERT INTO sales (receipt_no, subtotal, total, status, payment_timestamp, method, parent_receipt) VALUES (?, ?, ?, 'completed', ?, 'cash', ?)`, [receipt_no + '-CASH', splitCash, splitCash, finalTimestamp, receipt_no]);
            ...
            if (shiftResult.rows.length) {
                await runQuery('UPDATE shifts SET cash_sales = cash_sales + ? WHERE id = ?', [splitCash, shiftResult.rows[0].id]);
                await runQuery('UPDATE shifts SET mpesa_sales = mpesa_sales + ? WHERE id = ?', [splitMpesa, shiftResult.rows[0].id]);
            }
    ...
    try {
        await runQuery(`INSERT INTO sales (receipt_no, subtotal, total, status, payment_timestamp, method) VALUES (?, ?, ?, 'completed', ?, ?)`, [receipt_no, subtotal, subtotal, finalTimestamp, method]);
        ...
        for (const item of items) {
            ...
            await runQuery(`INSERT INTO sale_items (sale_id, product_id, qty, unit_price, total_price, cost_price) VALUES (?, ?, ?, ?, ?, ?)`, [saleId, item.id, item.qty, item.price, item.total, costPrice]);
            await runQuery('UPDATE products SET stock = stock - ? WHERE id = ?', [item.qty, item.id]);
        }
        const shiftResult = await runQuery('SELECT id FROM shifts WHERE is_active = 1 ORDER BY id DESC LIMIT 1');
        if (shiftResult.rows.length) {
            if (method === 'cash') {
                await runQuery('UPDATE shifts SET cash_sales = cash_sales + ? WHERE id = ?', [subtotal, shiftResult.rows[0].id]);
            } else if (method === 'mpesa') {
                await runQuery('UPDATE shifts SET mpesa_sales = mpesa_sales + ? WHERE id = ?', [subtotal, shiftResult.rows[0].id]);
                if (cash_refund && cash_refund > 0) {
                    await runQuery('UPDATE shifts SET cash_out = cash_out + ? WHERE id = ?', [cash_refund, shiftResult.rows[0].id]);
                    await runQuery('INSERT INTO refunds (sale_id, amount, reason) VALUES (?, ?, ?)', [saleId, cash_refund, 'M-Pesa overpayment cash refund']);
                }
            }
        }
...
app.post('/api/shift/open', async (req, res) => {
    ...
    try {
        await runQuery(`INSERT INTO shifts (user_id, opened_at, opening_cash, opening_mpesa, cash_sales, mpesa_sales, cash_out, expected_cash, expected_mpesa, is_active) VALUES (1, datetime('now'), ?, ?, 0, 0, 0, ?, ?, 1)`, [opening_cash || 0, opening_mpesa || 0, opening_cash || 0, opening_mpesa || 0]);
...
app.post('/api/settings', async (req, res) => {
    ...
    try {
        for (const [key, value] of Object.entries(settings)) {
            await runQuery('INSERT OR REPLACE INTO settings (key, value, updated_at) VALUES (?, ?, datetime("now"))', [key, String(value)]);
        }
    ...
    try {
        ...
        const insert = async (table, rows) => {
            ...
            for (const row of rows) {
                ...
                await runQuery(`INSERT INTO ${table} (${cols.join(',')}) VALUES (${placeholders})`, Object.values(row));
            }
        };
